<template>
  <div class="simAnalysis-device">
    <a-row :gutter="[8, 8]" class="device-row">
      <a-col :xs="24" :xl="9">
        <a-card title="设备利用率堆叠图" :headStyle="{ borderBottom: '0px solid' }" class="device-info-card fixed-card">
            <chart :option="trendOption" :chartData="trendData" />
        </a-card>
      </a-col>

      <a-col :xs="24" :xl="15">
        <a-card title="设备利用率时序图" :headStyle="{ borderBottom: '0px solid' }" class="machine-card fixed-card">
    ...
    <a-row :gutter="[8, 8]">
        <a-col :xs="24">
          <a-card title="主要设备利用率对比" :headStyle="{ borderBottom: '0px solid' }" class="machine-card gantt-card fixed-card">
