<template>
  ...
  <a-row :gutter="[8, 8]" class="device-row">
      <a-col :xs="24" :xl="9">
        <a-card title="性能趋势图" :headStyle="{ borderBottom: '0px solid' }" class="device-info-card fixed-card">
            <chart :option="trendOption" :chartData="trendData" />
        </a-card>
      </a-col>

      <a-col :xs="24" :xl="15">
        <a-card title="晶圆小时产能实时曲线图" :headStyle="{ borderBottom: '0px solid' }" class="machine-card fixed-card">
  ...
  <a-row :gutter="[8, 8]">
        <a-col :xs="24">
          <a-card title="机台腔室占用甘特图" :headStyle="{ borderBottom: '0px solid' }" class="machine-card gantt-card fixed-card">
