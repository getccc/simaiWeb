...
export const useKeybindingStore = defineStore("keybinding", () => {
  ...
  >(() => {
    return createKeybindingsByCommandId(
      Object.values(defaultKeybindings.value)
    );
  });
  ...
  function addDefaultKeybinding(keybinding: KeybindingImpl) {
    const serializedCombo = keybinding.combo.serialize();
    if (defaultKeybindings.value[serializedCombo]) {
      console.warn(`Keybinding on ${serializedCombo} already exists.`);
      return; // 或者抛出一个自定义错误
    }
  ...
  function loadExtensionKeybindings(extension: any) {
